...
export const updateUserById = async (id: string, data: UserUpdate): Promise<User | null> => {
  ...
  const setClauses = entries.map(([key], index) => `${fieldMap[key]} = $${index + 1}`)
...
export const deleteUserById = async (id: string): Promise<User | null> => {
  const { rows } = await pool.query<UserRow>('DELETE FROM users WHERE id = $1 RETURNING *', [id])
